...
and this [one about
**`tidyr`**](https://github.com/rstudio/cheatsheets/blob/main/data-import.pdf).
...
Some mouse gene have no human homologs. To visualise it easily, we will create 
a new table containing just the 2 columns `gene` and 
...
Pipes in R look like `%>%` (made available via the magrittr package) or `|>` 
(through base R). If you use RStudio, you can type the pipe with <kbd>Ctrl</kbd>
...
4. Identify genes associated with "abnormal DNA methylation" phenotype description, 
   and calculate the mean of their log expression at time 0, time 4 and time 8.
