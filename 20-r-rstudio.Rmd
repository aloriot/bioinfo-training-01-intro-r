...
*Summary of (dis)advantages of working with scripts:*

* (-) learning curve
* (+) share, comment code
* (+) redo quickly an analysis
* (+) reproducibility
...
3. Enter a name for this new folder (or "directory"), and choose a
   ...
   for this session (or whole training) (e.g., `bioinfo_training_intro`).
...
create directories (folders) for **scripts**, **data**, and **documents**.
 ...
 - **`scripts/`** (or `src`) This would be the location to keep your R
   ...
   separate folder for your functions.
...
* a `data/` folder to store our raw data,
* a `data_output/` folder for when we learn how to export data as CSV files, 
* and a `fig_output/` folder for the figures that we will save.

`r msmbstyle::question_begin()`

Under the `Files` tab on the right of the screen, click on `New Folder` and
create a folder named `data` within your newly created working directory
(e.g., `~/bioinfo_training_intro/data`). (Alternatively, type `dir.create("data")` at
...
The schema below represents the working directory `bioinfo_training_intro` with the
`data` and `fig_output` sub-directories, and 2 files in the latter:

```
bioinfo_training_intro/data/
                      /fig_output/fig1.pdf
                      /fig_output/fig2.png
```
...
absolute path `/home/user/bioinfo_training_intro/fig_output/fig1.pdf`.

If we were in the `data` directory, we would use the relative path
`../fig_output/fig1.pdf` or the same absolute path
`/home/user/bioinfo_training_intro/fig_output/fig1.pdf`.
...
* Help on functions and datasets through the *help* pages: RStudio help interface or commands `help(mean)` or `?mean`

* Asking for help when I get an error message or I don’t know how to achieve an operation:

    + your neighbour
    + more experimented colleague
    + [Stack Overflow with r tag](http://stackoverflow.com/questions/tagged/r)
    + [R-help mailing list](https://stat.ethz.ch/mailman/listinfo/r-help) or [topic-specific mailing lists](http://www.r-project.org/mail.html)
    + on a specific package: look at the description file, contact the author, open an issue on GitHub
...
R packages need to be : (1) installed (+ their depednencies), and (2) loaded in your environment
...
Another common package repository is [Github](https://github.com/). GitHub is a general-purpose online software
