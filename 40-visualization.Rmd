...
* Represent expression on the log10 scale; see
  ...
  now have a warning message appearing here?

`r msmbstyle::question_end()`

`r msmbstyle::solution_begin()`
```{r, eval=TRUE, purl=TRUE, echo=TRUE}
ggplot(data = rna,mapping = aes(x = expression_log))+ 
  geom_histogram() + 
  scale_x_log10()
```
`r msmbstyle::solution_end()`
...
- To use hexagonal binning `ggplot2`, first install the R package
...
```{r, eval = TRUE}
ggplot(data = rna, mapping = aes(y = expression_log,
                       x = sample)) +
...
Replace the box plot with a violin plot; see `geom_violin()`. Fill in the violins according to the time with `fill`.
...
and 'mean_exp', add a title to the figure and change the colour legend title:
