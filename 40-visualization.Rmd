...
over `sample` from the `rna` dataset with the mouse id showing in
different colors. Is this a good way to represent this type of data?
...
Let’s calculate the mean expression per duration of the infection for the 6 genes having the highest log fold changes comparing time 8 versus time 0.
First, we need to select the genes and create a subset of `rna` called `sub_rna` containing the 6 selected genes, then we need to group the data and calculate the mean gene expression within each group:

```{r}
rna_fc <- rna_fc %>% arrange(desc(time_8_vs_0))

genes_selected <- rna_fc$gene[1:6]
...
Use the `geom_col()` to try to reproduce the following figure starting from `rna` table.

```{r, echo = FALSE, expression_by_sample_facet-by-gene}
rna %>% 
  filter(gene %in% genes_selected) %>% 
  ggplot(aes(x = sample,  y = expression)) +
  geom_col(aes(fill = factor(time))) +
  facet_wrap(~ gene, scales = "free_y") +
...
```{r expression_by_sample_facet_solution}
rna %>% 
  filter(gene %in% genes_selected) %>% 
  ggplot(aes(x = sample,  y = expression)) +
  geom_col(aes(fill = factor(time))) +
